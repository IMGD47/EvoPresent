...
class ToFlow(nn.Module):
    ...
    def forward(self, input, style, feat, skip=None):
        ...
        device = input.device
        xs = (
            torch.tensor(xs, dtype=torch.float32, requires_grad=False, device=device)
            .unsqueeze(0)
...
class Direction(nn.Module):
    def __init__(self, motion_dim):
        ...
        # Cache Q at inference to avoid repeated linalg.qr (and lazy wrapper issues)
        self._q_cached = None  # type: ignore[var-annotated]

    def forward(self, input):
        weight = self.weight + 1e-8
        if self.training or (self._q_cached is None or getattr(self._q_cached, 'device', None) != weight.device):
            # Recompute in training or when cache missing/wrong device
            Q, R = torch.linalg.qr(weight)
            if not self.training:
                # Cache only for eval path
                with torch.no_grad():
                    self._q_cached = Q.detach()
        else:
            Q = self._q_cached  # type: ignore[assignment]
