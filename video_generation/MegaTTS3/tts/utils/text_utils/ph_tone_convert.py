...
def split_ph_timestamp(ph_timestamp):
    ...
    for idx, item in enumerate(ph_timestamp):
        if idx % 2 == 0:
            ...
            cur_timestamp = item
    # Sometimes the generated alignment tokens may end with a stray phone without a matching timestamp.
    # In that case, we trim the extra phone/tone to keep lengths consistent with durations.
    if len(ph_list) > len(dur_list):
        ph_list = ph_list[:len(dur_list)]
        tone_list = tone_list[:len(dur_list)]
    elif len(dur_list) > len(ph_list):
        # If durations are longer (should be rare), drop the last duration to match phones.
        dur_list = dur_list[:len(ph_list)]
